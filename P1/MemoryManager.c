 ...
 *      2.a Generate an array size - Generate a random number and map it to the range [1000 - 1500) using the utility function 'get_arr_size'.
 * 
...
int get_running_count()
{
  ...
  //1. 
	int iteration= get_iteration_count(random );
  	printf("[MemoryManager] Number of iterations: %d\n", iteration);
  //2. 

	for( int i=0; i< iteration; i++)
	{
	//a Generate an array size - Generate a random number and map it to the range [1000 - 1500) using the utility function 'get_arr_size'.
		
		int arrSize= get_arr_size(rand());
	//b) Allocate memory for the array in the heap.
	   	int *array = (int*)malloc(sizeof(int) * arrSize);
		
    //c) Populate the array with random numbers.
		for(int j=0; j<arrSize; j++)
		{
			array[j] = rand(); 
		}	 
	
    //d) Get the median of the array using the function 'return_median'.
	  	int Median =  return_median(array, arrSize);

	 //e) Check if the median is divisible by 13.
		if ( Median %13 ==0 )
		{
			DevisableBy13++;	
		}
	...
	//f Return the number of medians that were divisible by 13.	
	return DevisableBy13;
}
